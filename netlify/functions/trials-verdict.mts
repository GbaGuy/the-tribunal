...
import { buildFinalVerdictPrompt } from './_lib/prompts';
...
export default async (_req: Request, context: Context): Promise<Response> => {
  ...
  try {
    ...
    const panelOpinions = await sql`
      SELECT DISTINCT ON (r.persona_id) r.content, p.name
      FROM responses r
      JOIN personas p ON p.id = r.persona_id
    ...
    if (panelOpinions.length < 2) {
      await sql`UPDATE trials SET status = 'failed', completed_at = now() WHERE id = ${trialId}`;
      return errorResponse(422, 'Both panel judges must render an opinion before the final verdict');
    }

    const userMessage = buildFinalVerdictPrompt(
      theCase as any,
      characterResponses.map((r) => ({ name: r.name as string, seat: r.seat as string, content: r.content as string })),
      panelOpinions.map((r) => ({ name: r.name as string, content: r.content as string }))
    );
